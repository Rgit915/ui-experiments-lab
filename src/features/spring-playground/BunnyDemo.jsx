...
import { GRASS_COLORS, INITIAL_ITEMS, lerp } from "../../constants/Constants";
...
export default function BunnyDemo({ isDark }) {
  ...
  const hopTo = useCallback((startX, endX, duration) => {
    return new Promise((resolve) => {
      ...
      function frame(now) {
        ...
        setBunnyTransform(
          `translate(26,26) rotate(${tilt}) scale(${squishX},${squishY})`,
        );
  ...
  const nibble = useCallback((facingDir) => {
    return new Promise((resolve) => {
      ...
      const interval = setInterval(() => {
        ...
        const ty = n % 2 === 0 ? -4 : 2;
        setBunnyTransform(
          `translate(26,26) scaleX(${facingDir}) scale(1,${s}) translateY(${ty})`,
        );
        if (n >= 5) {
  ...
  const { top: grassTop, bottom: grassBottom } = isDark
    ? GRASS_COLORS.dark
    : GRASS_COLORS.light;
  ...
  return (
    <div
      ...
      style={{
        height: "200px",
        background: `linear-gradient(to bottom, ${grassTop} 0%, ${grassBottom} 100%)`,
      }}
    >
